            ...
            .sink { [weak self] completion in
                ...
                if let firstSeason = seasons.first,
                   let firstEpisode = firstSeason.episodes.first {
                    ...
                    self.loadVideoStream(voiceActing: voiceActing, season: firstSeason, episode: firstEpisode)
                } else {
                    self.error = String(localized: "key.no_episodes")
                    self.isLoading = false
                }
            }
    ...
    private func loadVideoStream(voiceActing: MovieVoiceActing, season: MovieSeason?, episode: MovieEpisode?) {
        ...
        .sink { [weak self] completion in
            ...
            if movieVideo.needPremium {
                self.error = String(localized: "key.premium_content")
                self.isLoading = false
                return
            }
            ...
            if defaultQuality == .ask || defaultQuality.rawValue == "ask" {
                // Вибираємо найкращу якість
                videoURL = movieVideo.getMaxQuality()
            } else {
            ...
            guard let url = videoURL else {
                self.error = String(localized: "key.video_url_not_available")
                self.isLoading = false
                return
            }
