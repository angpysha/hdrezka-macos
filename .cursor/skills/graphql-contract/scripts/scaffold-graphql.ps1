# scaffold-graphql.ps1 — create GraphQL schema stub + contract brief from REQ id.
param(
...
if ($Session) {
    ...
    $apiDir = Get-ArtifactDir $RepoRoot 'graphql'
...
if ([string]::IsNullOrWhiteSpace($FeatureTitle)) {
    ...
    else { $FeatureTitle = "REQ-$nnnn GraphQL API" }
}

$outSchema = Join-Path $apiDir "GQL-$nnnn-$Slug.graphql"
$outBrief = Join-Path $apiDir "GQL-$nnnn-$Slug-brief.md"

if ((Test-Path $outSchema) -and -not $Force) {
    Write-Host "FAIL: $outSchema exists (use -Force)" -ForegroundColor Red
    exit 2
}

$stub = Get-Content (Join-Path $PSScriptRoot '../templates/schema-stub.graphql') -Raw
$stub = $stub.Replace('{REQ_ID}', $nnnn).Replace('{FEATURE_TITLE}', $FeatureTitle)
Set-Content -Path $outSchema -Value $stub -Encoding UTF8
...
Write-Host "OK   GraphQL schema -> $outSchema"
Write-Host "OK   Contract brief -> $outBrief"
Write-Host 'PASS: GraphQL scaffold complete'
exit 0
